...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
        ...
        elif m in {CBLinear, TorchVision, Index}:   # 此处增加CARAFE模块
            c2 = args[0]
            c1 = ch[f]
            args = [c1, c2, *args[1:]]
        elif m is CBFuse:
            ...
            # 获取输入通道数（假设输入来自前一层的输出）
            c1 = ch[f]  # f是当前层的输入来源层索引（可能为单层或多层）

            # 解析参数（假设yaml中配置为 [channels, gamma, b]）
            # 例如：args = [128, 2, 1] 或 [256]
            # 如果args未指定gamma和b，则使用默认值
            if len(args) < 3:
                args = [c1] + list(args) + [2, 1][len(args):]  # 自动补全默认参数
        else:
            c2 = ch[f]
